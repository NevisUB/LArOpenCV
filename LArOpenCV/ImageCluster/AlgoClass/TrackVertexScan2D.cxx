#ifndef __TRACKVERTEXSCAN2D_CXX__
#define __TRACKVERTEXSCAN2D_CXX__

#include "TrackVertexScan2D.h"
...
namespace larocv{
  ...
  void TrackVertexScan2D::ExtremePoints(const std::vector<float>& array,
  ...
  float TrackVertexScan2D::AngularSpread(const ::cv::Mat& polarimg,
  ...
  data::CircleVertex TrackVertexScan2D::RadialScan(const cv::Mat& img, const geo2d::Vector<float>& pt)
  {
  ...
  data::CircleVertex TrackVertexScan2D::TwoPointInspection(const cv::Mat& img, const geo2d::Vector<float>& pt)
  {
  ...
  double TrackVertexScan2D::CircleWeight(const larocv::data::CircleVertex& cvtx)
  {
  ...
  void TrackVertexScan2D::AnalyzePlane(const ::cv::Mat& img,
  ...
  void TrackVertexScan2D::XPlaneTimeScan(const std::vector<const cv::Mat>& img_v)
  {
    ...
    // Initialize results
    _time_binned_score_vv.resize(img_v.size());
    for(size_t plane=0; plane<img_v.size(); ++plane)
      _time_binned_score_vv[plane].clear();
	...
	*/
    ...
    for(size_t plane=0; plane<img_v.size(); ++plane) {
      _time_binned_score_vv[plane].resize(num_ticks+1);
      for(size_t tick=0; tick<=num_ticks; ++tick)
	_time_binned_score_vv[plane][tick] = -1;
    }
    ...
    for(size_t plane=0; plane<img_v.size(); ++plane) {
      ...
      auto const& tick_offset    = _geo.x_offset(plane);
	...
	auto& time_binned_score = _time_binned_score_vv[plane][tick_idx];
	...
	LAROCV_DEBUG() << "    Plane " << plane << " dtheta = " << _time_binned_score_vv[plane][tick] << std::endl;
	if(_time_binned_score_vv[plane][tick] < 0) continue;
	dtheta_sum += _time_binned_score_vv[plane][tick];
	if(_time_binned_score_vv[plane][tick] < dtheta_min)
	  dtheta_min = _time_binned_score_vv[plane][tick];
	num_valid_planes +=1;
      }
      ...
      _time_binned_score0_v[tick] = dtheta_min;

      if(num_valid_planes < 2) continue;
      //_time_binned_score1_v[tick] = dtheta_sum;
      _time_binned_score1_v[tick] = dtheta_min;
    }

    return;
  }

  void TrackVertexScan2D::XPlaneTimeProposal()
  {
				       ...
				       std::vector<std::vector<larocv::data::CircleVertex> >& vtx2d_vv)
  {
	...
	for(size_t circle_idx=0; circle_idx<plane_data._circle_scan_v.size(); ++circle_idx) {
	  ...
	  dist = std::fabs(circle.center.x - _geo.x_offset(plane) - target_tick);
	...
	vtx2d_vv.emplace_back(std::move(circle_vtx_v));
      }
    }
  }
  ...
  void TrackVertexScan2D::XPlaneWireProposal() 
  {
    ...
    */
	...
	for(size_t minidx1_idx=0; minidx1_idx<wire_binned_minidx1_v.size(); ++minidx1_idx) {
	  ...
	  for(size_t minidx2_idx=0; minidx2_idx<wire_binned_minidx2_v.size(); ++minidx2_idx) {
		...
		float tick_dist = std::fabs( (circle1.center.x - _geo.x_offset(seed_plane1) )
					     -
					     (circle2.center.x - _geo.x_offset(seed_plane2) )
					     );
					     ...
					     std::vector<std::vector<larocv::data::CircleVertex> >& vtx2d_vv)
  {
    ...
    XPlaneWireProposal();
    WireVertex3D(img_v,vtx3d_v,vtx2d_vv);
  }
