#ifndef __LINEARPARTICLECLUSTER_CXX__
#define __LINEARPARTICLECLUSTER_CXX__

#include "LinearParticleCluster.h"
...
namespace larocv {
  ...
  LinearParticleCluster::LinearParticleCluster()
    : laropencv_base("LinearParticleCluster"),
      _geo()
  {
    set_verbosity(msg::kNORMAL);

    _edges_from_mean = true;
    _pi_threshold = 10;

    _min_size_track_ctor = 10;
    _min_length_track_ctor = 6;
  ...
  void LinearParticleCluster::Configure(const Config_t &pset)
  {
    _min_size_track_ctor = pset.get<size_t>("TrackContourSizeMin",10);
    _min_length_track_ctor = pset.get<float>("TrackContourLengthMin",6);    
    
  }

  void LinearParticleCluster::EdgesFromMeanValue(const GEO2D_Contour_t& ctor,
  ...
  void LinearParticleCluster::EdgesFromPCAProjection(const cv::Mat& img,
						  ...
						  geo2d::Vector<float>& edge2) const
  {
    ...
    try  {
      ...
      LAROCV_DEBUG() << "Infinite PCA detected" << std::endl;
  ...
  void LinearParticleCluster::FindEdges(const cv::Mat& img,
				     const GEO2D_Contour_t& ctor,
				     geo2d::Vector<float>& edge1,
				     geo2d::Vector<float>& edge2) const
  {
  ...
  LinearParticleCluster::FindLinearTrack2D(const cv::Mat& img) const
  {
  ...
  LinearParticleCluster::FindLinearTrack(const std::vector<std::vector<larocv::data::LinearTrack2D> >& strack_vv)
  {
