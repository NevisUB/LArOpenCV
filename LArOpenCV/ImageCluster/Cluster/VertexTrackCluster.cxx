...
namespace larocv {
							 ...
							 larocv::ROI& roi)
  {
    ...
    for(size_t vtx_id = 0; vtx_id < data._vtx_cluster_v.size(); ++vtx_id) {
      ...
      auto contour_v = _algo.CreateParticleCluster(img,circle_vtx);
      ...
      auto const parent_ctor_id = _algo.FindSuperCluster(circle_vtx.center);
      geo2d::VectorArray<int> parent_points;
      if(parent_ctor_id != kINVALID_SIZE) {
	auto const& parent_ctor = _algo.SuperClusters()[parent_ctor_id];
