...
namespace imutil {
  namespace convert {
    ...
    NumpyAllocator* NumpyAllocator::_me = nullptr;
    ...
    void NumpyAllocator::deallocate(::cv::UMatData* data) const
	...
	{
	  if( !m.data )
	    m.allocator = &NumpyAllocator::get();
	}
      ...
      if( m.data )
	{
	  ...
	  //for(int i=0; i<(*(refcountFromPyObject(o))); ++i) m.addref();
	...
	{
	  Mat tmp;
	  tmp.allocator = &NumpyAllocator::get();
	  transpose(m, tmp);
	  m = tmp;
	}
      return m;
	...
	Py_RETURN_NONE;
      Mat temp, *p = (Mat*)&m;
      if(!p->u->refcount || p->allocator != &NumpyAllocator::get())
	{
	  temp.allocator = &NumpyAllocator::get();
	  m.copyTo(temp);
	  p = &temp;
	}
      p->addref();
