#ifndef __XSTRACKCLUSTER_CXX__
#define __XSTRACKCLUSTER_CXX__

#include "XsTrackCluster.h"
#include "Refine2DVertex.h"

namespace larocv {

  /// Global larocv::XsTrackClusterFactory to register AlgoFactory
  static XsTrackClusterFactory __global_VERTEXTRACKClusterFactory__;

  void XsTrackCluster::_Configure_(const ::fcllite::PSet &pset)
  {
  ...
  larocv::Cluster2DArray_t XsTrackCluster::_Process_(const larocv::Cluster2DArray_t& clusters,
							 const ::cv::Mat& img,
							 ...
							 larocv::ROI& roi)
  {
    ...
    auto& data = AlgoData<larocv::XsTrackClusterData>();
    ...
    for(int xs_pt_idx=0; xs_pt_idx<ref_xs_v.size(); ++xs_pt_idx) {
      ...
      auto angle = angle1 < 0 ? angle1+180-360. : angle1+180;
	...
	for(size_t col=0; col<mask_col_max; ++col) {
      ...
      */
      ...
      //row_min = (size_t)((float)(rot_polarimg.rows) * (0.5 - 10./360.)); // was 10/360.
      //row_max = (size_t)((float)(rot_polarimg.rows) * (0.5 + 10./360.));

      row_min = (size_t)((float)(rot_polarimg.rows) * (0.5 - dangle10/360.)); // was 10/360.
      row_max = (size_t)((float)(rot_polarimg.rows) * (0.5 + dangle21/360.));
