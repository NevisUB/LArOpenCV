...
namespace larocv{
				 ...
				 larocv::ROI& roi)
  {
    ...
    auto const& pca_ctor_lines_vv   = pcacandidates_data._ctor_lines_v_v_v[meta.plane()];
	...
	if (atomic_index_v_pair.size()==1) {
	  ...
	  */
	  ...
	  LAROCV_DEBUG()<<"(should be same as atomic size on this track )size of ctor line is " <<pca_ctor_lines_vv[i].size()<<std::endl;

	  for (auto& pt : pts){
	    auto & ctor = clusters[atomic_index_v_pair[k].first]._contour;
	    if( cv::pointPolygonTest(ctor, pt, false) < 0 ) continue;
	    
	    auto & pca = pca_ctor_lines_vv[i][atomic_index_v_pair[k].first]; //trying to get pca from ordering PCACandidate
